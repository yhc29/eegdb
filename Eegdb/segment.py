...
class Segment:
  def __init__(self,subjectid,fileid,vendor,file_type,channel_index,channel_label,sample_rate,start_datetime,end_datetime,signals,time_points=None,segment_datetime=None,segment_duration=None):
    self.__doc = {
      "subjectid": subjectid,
      "fileid": fileid,
      "vendor": vendor,
  ...
  def generate_mongo_doc(self):
    ...
    while _clip_datetime <= _end_datetime:
      ...
      try:
        _signals_dict[str(_minute)]
      except:
        _signals_dict[str(_minute)] = {}
      _signals_dict[str(_minute)][str(_second)]={"values":_clip_signals}
